...
const Members: React.FC<MembersProps> = ({ members, onAddMember, onUpdateMember, onDeleteMember }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-end mb-10" id="members-header">
        <div>
          <h2 className="font-serif text-5xl text-slate-900 mb-2" id="members-title">Library Members</h2>
          <p className="text-slate-500 text-lg font-outfit" id="members-subtitle">Manage your library membership</p>
        </div>
        <button 
      ...
      <div className="mb-10" id="members-search-container">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="members-grid">
        {filteredMembers.map((member) => (
          <div key={member.id} className="bg-white rounded-xl border border-slate-100 p-6 shadow-sm hover:shadow-md transition-all group relative" id={`member-card-${member.id}`}>
            <button 
            ...
            <div className="absolute top-16 right-6 text-slate-400 font-bold text-sm" id={`member-books-${member.id}`}>
              {member.borrowedCount} books
            </div>
            
            <div className="flex gap-6 items-start" id={`member-content-${member.id}`}>
              <div className="w-16 h-16 bg-[#F2ECE4] rounded-full flex items-center justify-center font-serif text-xl font-bold text-[#712A2A]" id={`member-avatar-${member.id}`}>
                {member.name.split(' ').map(n => n[0]).join('')}
              </div>
              
              <div className="space-y-3" id={`member-info-${member.id}`}>
                <div>
